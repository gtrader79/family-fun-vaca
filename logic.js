...
function runSimulationController() {
    ...
    *******************************/
        const matchups = [
            {
                name: "Passing Attack vs Secondary",
                gap: Math.abs(getZScore(teamAMetrics.passOff, metrics.offPass) - getZScore(teamBMetrics.passDef, metrics.defPass, true)),
                description: `${teamA.teamName}'s air game against ${teamB.teamName}'s pass defense.`
            },
            {
                name: "Ground War",
                gap: Math.abs(getZScore(teamAMetrics.rushOff, metrics.offRush) - getZScore(teamBMetrics.rushRush) - getZScore(teamBMetrics.rushDef, metrics.defRush, true)),
                description: `the battle between ${teamA.teamName}'s rushers and ${teamB.teamName}'s front seven.`
            }
            // Add more here (e.g., Turnovers, Special Teams) as you build them
        ];
        
        // Sort by the 'gap' descending to find the biggest outlier
        matchups.sort((a, b) => b.gap - a.gap);
        
        const topMatchup = matchups[0];
        const xFactorNarrative = `<strong>${topMatchup.name}:</strong> The simulation is most sensitive to ${topMatchup.description}`;
